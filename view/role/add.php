...
@foreach ($ability_config['role']['read'] as $read_key => $read_name)
            <input class="tr-select-all-children" type="checkbox" id="{{ $model_key.'-'.$read_key }}" name="{{ $model_key.'['.$read_key.']' }}"><label for="{{ $model_key.'-'.$read_key }}">{{ $read_name }}</label>
@endforeach
        </td>
        <td>
@foreach ($ability_config['role']['write'] as $write_key => $write_name)
            <input class="tr-select-all-children" type="checkbox" id="{{ $model_key.'-'.$write_key }}" name="{{ $model_key.'['.$write_key.']' }}"><label for="{{ $model_key.'-'.$write_key }}">{{ $write_name }}</label>
@endforeach
        </td>
        <td>
