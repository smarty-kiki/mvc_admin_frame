...
@endforeach
                <th>
                    @^^if (role_ability_need('{{ $entity_name }}_setting.{{ $entity_name }}_add')) <a href='/{{ english_word_pluralize($entity_name) }}/add'>添加</a> @^^endif
                </th>
            </tr>
        </thead>
        <tbody>
...
@endforeach
                <td>
                    @^^if (role_ability_need('{{ $entity_name }}_setting.{{ $entity_name }}_detail')) <a href='/{{ english_word_pluralize($entity_name) }}/detail/^^{^^{ ${{ $entity_name }}->id ^^}^^}'>详情</a> @^^endif
                    @^^if (role_ability_need('{{ $entity_name }}_setting.{{ $entity_name }}_update')) <a href='/{{ english_word_pluralize($entity_name) }}/update/^^{^^{ ${{ $entity_name }}->id ^^}^^}'>修改</a> @^^endif
                    @^^if (role_ability_need('{{ $entity_name }}_setting.{{ $entity_name }}_delete')) <a href='javascript:delete_^^{^^{ ${{ $entity_name }}->id ^^}^^}.submit();'>删除</a> @^^endif
