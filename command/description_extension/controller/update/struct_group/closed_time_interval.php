...
@endphp
    ...
    if ({{ $range_datetime_variable_name }}) {
        ...
        ${{ $entity_name }}->{{ $struct_name_map['$(name)_start_time'] }} = {{ $range_datetime_explode_variable_name }}[0];
        ${{ $entity_name }}->{{ $struct_name_map['$(name)_end_time'] }} = {{ $range_datetime_explode_variable_name }}[1];
    }
